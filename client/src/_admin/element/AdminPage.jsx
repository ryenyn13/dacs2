...
const AdminPage = () => {
  ...
  const {mutate: addCake,isError,error,isAdding,} = useMutation({
  ...
  const handleAddSubmit = (e) => {
   ...
   // const handleAddSubmit = (e) => {
  //   e.preventDefault();
  //   const updatedFormData = { ...formData, images: imgs };
  //   addCake(updatedFormData); // Thêm bánh kem
  // };
  ...
  const { mutate: editCake, isLoading: isEditing } = useMutation({
    mutationFn: async (updatedData) => {
      try {
        const res = await fetch(`/api/cake/editCake/${cakeData.id}`, {
          method: 'PUSH',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  //     addCake(updatedFormData); // Thêm bánh kem
  //   } else {
  //     editCake(updatedFormData); // Chỉnh sửa bánh kem
  //   }
  // };
  // Hàm submit cho modal Add

  // Hàm submit cho modal Edit
